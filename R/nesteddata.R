...
#'
#' @param df Wide data frame with one column for each factor and one for each dimension of the
#' response variable.
#' @param levels Character vector containing the names of the factor variables. Must be supplied in
#' the bottom-up order of nesting so that `level[i]` is nested inside `level[i+1]`.
#'
