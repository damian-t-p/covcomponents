...
#' @param arg_conditioner A function that computes the conditional expectation of arguments required
#' for the `crit_argmax` function. Should take as its first argument the updating parameter values.
...
#' @param arg_conditioner_params List of non-updating arguments to pass to `arg_conditioner`
#'
...
unbalanced_EM <- function(crit_argmax,
                          ...
                          arg_conditioner_params = list()) {

  prev_params <- init_params

  for (i in 1:max.iter) {

    cond_args <- rlang::inject(
      arg_conditioner(prev_params, !!!arg_conditioner_params)
    )

    curr_params <- rlang::inject(
      crit_argmax(cond_args, !!!crit_argmax_params)
    )
